...
export class Hydrozoa_02 {
    ...
    create() {
        ...
        for (let i = 0; i < this.centralSpine.nodeCount; i++) {
            ...
            this.armsRight.push(new VerletStrand_2N(this.p, h1, t1, 15, new p5.Vector(.2, .4), new ProtoStyle(this.p, this.p.color(100, 100, 135), this.p.color(200, 200, 255), .8, 3)));

            let h2 = h1.copy();
            let t2 = new p5.Vector(h2.x, h2.y - this.armLength, h2.z)
            this.armsLeft.push(new VerletStrand_2N(this.p, h2, t2, 15, new p5.Vector(.2, .4), new ProtoStyle(this.p, this.p.color(100, 100, 135), this.p.color(200, 200, 255), .8, 3)));
    ...
    move(bounds?: p5.Vector) {
        ...
        this.move(bounds);
        this.draw(true, true);
        // this.drawBounds(p.color(200, 140, 150, 80));
        // this.drawBoundsOutline(p.color(200, 140, 150), 1);
        ...
        for (let i = 0; i < this.centralSpine!.nodeCount; i++) {
            ...
            this.armsRight[i].draw(false, true);
            ...
            if (i > 0) {
                ...
                p.line(
                    ...
                    this.armsRight[i].centralSpine!.nodes[armEndNodeIndex].pos.x,
                    this.armsRight[i].centralSpine!.nodes[armEndNodeIndex].pos.y,
                    this.armsRight[i].centralSpine!.nodes[armEndNodeIndex].pos.z
                );
                // left connections
                p.line(

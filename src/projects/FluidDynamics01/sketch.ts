...
import p5 from "p5";
import { FluidDynamics01 } from './FluidDynamics01';
...
const sketch = (p: p5) => {
    ...
    p.setup = () => {
        ...
        document.title = "FluidDynamics01";
        ...
        fluidDyn = new FluidDynamics01(p);
    };
    ...
    p.draw = () => {
        ...
        // p.orbitControl();
        p.translate(-fluidDyn.width / 2, -fluidDyn.height / 2, 0);
    ...
    p.keyTyped = () => {
        if (p.key === 'p') {
            const name = "FluidDynamics01" + "_" + p.year() + p.month() + p.day() + p.hour() + p.minute() + p.second() + ".png";
            p.save(name);
        }
    }

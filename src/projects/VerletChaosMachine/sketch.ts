...
import p5 from "p5";
import { VerletChaosMachine } from './VerletChaosMachine';
...
const sketch = (p: p5) => {
    ...
    p.setup = () => {
        ...
        document.title = "VerletChaosMachine";
    ...
    p.keyTyped = () => {
        if (p.key === 'p') {
            const name = "VerletChaosMachine" + "_" + p.year() + p.month() + p.day() + p.hour() + p.minute() + p.second() + ".png";
            p.save(name);
        }
    }
